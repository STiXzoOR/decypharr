...
class RepairManager {
  ...
  bindEvents() {
    // Form submission
    this.refs.repairForm.addEventListener("submit", (e) =>
      this.handleFormSubmit(e),
    );

    // Jobs table events
    this.refs.refreshJobs.addEventListener("click", () => this.loadJobs());
    this.refs.deleteSelectedJobs.addEventListener("click", () =>
      this.deleteSelectedJobs(),
    );
    this.refs.selectAllJobs.addEventListener("change", (e) =>
      this.toggleSelectAllJobs(e.target.checked),
    );

    // Modal events
    this.refs.processJobBtn.addEventListener("click", () =>
      this.processCurrentJob(),
    );
    this.refs.stopJobBtn.addEventListener("click", () => this.stopCurrentJob());

    // Filter events
    this.refs.itemSearchInput.addEventListener(
      "input",
      window.decypharrUtils.debounce(() => this.applyFilters(), 300),
    );
    this.refs.arrFilterSelect.addEventListener("change", () =>
      this.applyFilters(),
    );
    this.refs.pathFilterSelect.addEventListener("change", () =>
      this.applyFilters(),
    );
    this.refs.clearFiltersBtn.addEventListener("click", () =>
      this.clearFilters(),
    );

    // Table row events (using event delegation)
    this.refs.jobsTableBody.addEventListener("click", (e) =>
      this.handleJobTableClick(e),
    );
    this.refs.brokenItemsTableBody.addEventListener("click", (e) =>
      this.handleItemTableClick(e),
    );
  }
  ...
  async handleFormSubmit(e) {
    ...
    try {
      ...
      window.decypharrUtils.createToast(
        `Error starting repair: ${error.message}`,
        "error",
      );
    } finally {
  ...
  createJobRow(job) {
    ...
    const totalItems = job.broken_items
      ? Object.values(job.broken_items).reduce(
    ...
    const isActive = ["started", "processing"].includes(job.status);

    // Build progress display - data comes from our trusted backend API
    let progressDisplay = "";
    if (isActive && job.total_items > 0) {
      const checkedOrProcessed =
        job.status === "started" ? job.checked_items : job.processed_items;
      progressDisplay = `
                <div class="flex items-center gap-2">
                    <progress class="progress progress-primary w-16 h-2" value="${progress}" max="100"></progress>
                    <span class="text-xs text-base-content/70">${checkedOrProcessed}/${job.total_items}</span>
                </div>
            `;
    } else if (job.status === "completed") {
      progressDisplay = `<span class="text-xs text-success">Done</span>`;
    } else if (job.status === "failed" || job.status === "cancelled") {
      progressDisplay = `<span class="text-xs text-error">-</span>`;
    } else if (job.status === "pending") {
      progressDisplay = `<span class="text-xs text-warning">Waiting</span>`;
    } else if (isActive) {
      progressDisplay = `<span class="loading loading-spinner loading-xs"></span>`;
    }

    // Note: All dynamic content comes from our trusted backend API
    row.innerHTML = `
            <td>
  ...
  renderJobsPagination(totalPages) {
    ...
    let startPage = Math.max(
      1,
      this.state.currentPage - Math.floor(maxPageButtons / 2),
    );
  ...
  populateJobModal(job) {
    ...
    this.refs.modalJobMediaIds.textContent =
      job.media_ids && job.media_ids.length > 0
        ? job.media_ids.join(", ")
        : "All media";
    this.refs.modalJobAutoProcess.textContent = job.auto_process ? "Yes" : "No";

    // Dates
    this.refs.modalJobStarted.textContent = new Date(
      job.created_at,
    ).toLocaleString();
    ...
    // Action buttons
    this.refs.processJobBtn.classList.toggle(
      "hidden",
      job.status !== "pending",
    );
    this.refs.stopJobBtn.classList.toggle(
      "hidden",
      !["started", "processing"].includes(job.status),
    );
  ...
  getFileType(path) {
    ...
    if (movieExtensions.some((ext) => pathLower.endsWith(ext))) {
      return pathLower.includes("/movies/") || pathLower.includes("/films/")
        ? "movie"
        : "tv";
    }
  ...
  populateArrFilter() {
    ...
    const uniqueArrs = [
      ...new Set(this.state.allBrokenItems.map((item) => item.arr)),
    ];
  ...
  applyFilters() {
    ...
    this.state.filteredItems = this.state.allBrokenItems.filter((item) => {
      const matchesSearch =
        !searchTerm || item.path.toLowerCase().includes(searchTerm);
  ...
  renderBrokenItemsTable() {
    ...
    const totalPages = Math.ceil(
      this.state.filteredItems.length / this.state.itemsPerModalPage,
    );
    const startIndex =
      (this.state.currentItemsPage - 1) * this.state.itemsPerModalPage;
    const endIndex = Math.min(
      startIndex + this.state.itemsPerModalPage,
  ...
  renderItemsPagination(totalPages) {
    ...
    let startPage = Math.max(
      1,
      this.state.currentItemsPage - Math.floor(maxButtons / 2),
    );
  ...
  updateItemsStats() {
    ...
    this.refs.modalFooterStats.textContent = `Total: ${this.state.allBrokenItems.length} | Filtered: ${this.state.filteredItems.length}`;
  }
  ...
  async deleteSelectedJobs() {
    ...
    if (
      !confirm(`Are you sure you want to delete ${selectedIds.length} job(s)?`)
    )
      return;
    ...
    try {
      ...
      window.decypharrUtils.createToast(
        `${selectedIds.length} job(s) deleted successfully`,
        "success",
      );
      await this.loadJobs();
    } catch (error) {
      ...
      window.decypharrUtils.createToast(
        `Error deleting jobs: ${error.message}`,
        "error",
      );
    }
  }
  ...
  updateJobSelectionState() {
    ...
    const enabledBoxes = document.querySelectorAll(
      ".job-checkbox:not(:disabled)",
    );
  ...
  getJobProgress(job) {
    // Use real progress tracking from the backend
    if (job.status === "completed") return 100;
    if (job.status === "failed" || job.status === "cancelled") return 0;

    // For scanning phase (started status), use checked_items / total_items
    if (job.status === "started" && job.total_items > 0) {
      return Math.round((job.checked_items / job.total_items) * 100);
    }

    // For processing phase, use processed_items / total_items
    if (job.status === "processing" && job.total_items > 0) {
  ...
  searchJobs(searchTerm) {
    ...
    return this.state.jobs.filter(
      (job) =>
        ...
        (job.media_ids &&
          job.media_ids.some((id) => id.toString().includes(term))),
    );
  }
...
const RepairUtils = {
  ...
  generateRepairSummary(job) {
    ...
    const itemCounts = Object.entries(job.broken_items).map(
      ([arr, items]) => `${arr}: ${items.length} items`,
    );

    const totalItems = Object.values(job.broken_items).reduce(
      (sum, arr) => sum + arr.length,
      0,
    );
